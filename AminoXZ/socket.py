...
class SocketHandler:
	...
	def run_amino_socket(self):
		try:
			...
			final = f"{self.client.deviceId}|{int(time() * 1000)}"

			self.headers = {
				"NDCDEVICEID": self.generator.generateDeviceId() if self.client.autoDevice else self.client.deviceId,
				"NDCAUTH": f"sid={self.client.sid}",
				"NDC-MSG-SIG": self.generator.signature(final)
			}
